...
import { remove_To_Cart } from '../reduxSlice/Cart_Slice';
            ...
            {cartItem.map((product) => {
              return (
                <div key={product.id} className="border border-black h-full p-4">
                  ...
                  <button className="text-[#88BDBC] border bg-[#254E58] px-4 py-1 rounded hover:text-[#254E58] hover:bg-[#88BDBC]" onClick={() => dispatch(remove_To_Cart(product))}>RemoveToCart</button>
                </div>
              )
            })}
          </div>
